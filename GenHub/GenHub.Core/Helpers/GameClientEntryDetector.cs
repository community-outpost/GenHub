...
public static class GameClientEntryDetector
{
    ...
    private static readonly HashSet<string> NonExecutableDataExtensions = new(StringComparer.OrdinalIgnoreCase)
    {
        ".big", ".ini", ".txt", ".wav", ".mp3", ".w3d", ".dds", ".tga", ".bmp", ".png",
        ".jpg", ".jpeg", ".bik", ".csf", ".map", ".str", ".cfg", ".xml", ".json", ".doc",
        ".rtf", ".pdf", ".dll", ".so", ".dylib", ".la", ".a", ".pdb", ".log", ".bak",
        ".htm", ".html", ".md",
    };
    ...
    {
        var (natives, flatpaks, windowsExecutables) = ScanFlatPayloadCandidates(payloadRoot, cancellationToken);
        ...
        var resolution = ResolveCandidateSet(payloadRoot, natives, "only native binary", "known primary game binary", "native binaries", preferKnown: true)
            ?? ResolveCandidateSet(payloadRoot, flatpaks, "only Flatpak bundle", string.Empty, "Flatpak bundles", preferKnown: false)
            ?? ResolveCandidateSet(payloadRoot, windowsExecutables, "only Windows executable", "known primary Windows executable", "Windows executables", preferKnown: true);

        return resolution ?? EntryPointResolution.Failed("Payload contains no launchable file.", Array.Empty<string>());
    }

    private static (List<string> Natives, List<string> Flatpaks, List<string> WindowsExecutables) ScanFlatPayloadCandidates(string root, CancellationToken cancellationToken)
    {
        var natives = new List<string>();
        var flatpaks = new List<string>();
        var windowsExecutables = new List<string>();
        ...
        try
        {
            foreach (var file in Directory.EnumerateFiles(root, "*", RecursiveOptions))
            {
                ...
                {
                    windowsExecutables.Add(file);
                    continue;
                }

                if (ExecutableFileClassifier.IsLibraryFile(file))
                {
                    continue;
                }

                var ext = Path.GetExtension(file);
                if (!string.IsNullOrEmpty(ext) && NonExecutableDataExtensions.Contains(ext))
                {
                    continue;
                }
        ...
        return (natives, flatpaks, windowsExecutables);
    }
