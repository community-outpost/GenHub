...
- **`targetGame` supports both numeric and string enum formats.** `GameType` is configured with
  `JsonStringEnumConverter`, so both integer values (`0 = Generals`, `1 = ZeroHour`, `2 = Unknown`)
  and string names like `"ZeroHour"` deserialize properly. All 12 items in this catalog target Zero Hour (`1`).
  target Zero Hour (`1`).
