    ...
    IGameInstallationService? installationService = null) : ObservableObject
{
    ...
    {
        return (client.PublisherType is { } pub && pub.Contains(ReplayManagerConstants.RetailKeyword, StringComparison.OrdinalIgnoreCase)) ||
               client.Name.Contains(ReplayManagerConstants.RetailKeyword, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private void AddMatchedClientCard(CrcMappingEntry matchedClient, GameType targetGame, ISet<string> discoveredKeys)
    {
        ...
        var card = new GameClientCardViewModel(new GameClientCardParameters(
            ...
            Name: matchedClient.Description ?? $"{matchedClient.Publisher} {matchedClient.Version}",
            Version: matchedClient.Version ?? PublisherInfoConstants.Unknown.Name,
            Publisher: matchedClient.Publisher ?? PublisherInfoConstants.Unknown.Name,
    ...
    {
        if (discoveredKeys.Contains(ReplayManagerConstants.RetailBaseClientKey))
        {
            return true;
        }
    ...
    private void AddRetailClientCard(GameType targetGame, ReplayFile? replay, ISet<string> discoveredKeys)
    {
        ...
        var retailClient = new GameClient
        {
            ...
            PublisherType = PublisherTypeConstants.Retail,
            GameType = targetGame,
        };
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            foreach (var manifest in manifestsResult.Data.Where(m => m.ContentType == ContentType.GameClient))
            {
                ...
                if (isCrcMatch && (manifest.Id.Value.Contains(ReplayManagerConstants.RetailKeyword, StringComparison.OrdinalIgnoreCase) ||
                                   (manifest.Publisher?.PublisherType is { } pubType && pubType.Contains(ReplayManagerConstants.RetailKeyword, StringComparison.OrdinalIgnoreCase))))
                {
                    discoveredKeys.Add(ReplayManagerConstants.RetailBaseClientKey);
                }
            }
