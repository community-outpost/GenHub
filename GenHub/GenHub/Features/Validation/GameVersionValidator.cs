...
public class GameVersionValidator : FileSystemValidator, IGameVersionValidator, IValidator<GameVersion>
{
    ...
    private async Task<List<ValidationIssue>> ValidateGameVersionSpecificAsync(GameVersion gameVersion, GameManifest manifest, CancellationToken cancellationToken)
    {
        var issues = new List<ValidationIssue>();
