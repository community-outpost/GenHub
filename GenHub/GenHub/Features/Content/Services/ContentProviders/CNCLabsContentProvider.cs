...
public class CNCLabsContentProvider : BaseContentProvider
{
    ...
    /// <inheritdoc />
    protected override Task<ContentOperationResult<GameManifest>> PrepareContentInternalAsync(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return Task.FromResult(ContentOperationResult<GameManifest>.CreateSuccess(manifest));
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Failed to prepare CNC Labs content for manifest {ManifestId}", manifest.Id);
            return Task.FromResult(ContentOperationResult<GameManifest>.CreateFailure($"CNC Labs content preparation failed: {ex.Message}"));
        }
    }
}
