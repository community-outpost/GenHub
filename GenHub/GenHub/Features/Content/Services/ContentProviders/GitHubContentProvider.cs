...
public class GitHubContentProvider : BaseContentProvider
{
    ...
    /// <inheritdoc />
    public override async Task<ContentOperationResult<GameManifest>> GetContentAsync(
        string contentId, CancellationToken cancellationToken = default)
    {
        ...
        {
            return ContentOperationResult<GameManifest>.CreateFailure($"Content not found: {contentId}");
        }

        var result = searchResult.Data!.First();
        var manifest = result.GetData<GameManifest>();

        return manifest != null
            ? ContentOperationResult<GameManifest>.CreateSuccess(manifest)
            : ContentOperationResult<GameManifest>.CreateFailure("Manifest not available in search result");
    }

    /// <inheritdoc />
    protected override async Task<ContentOperationResult<GameManifest>> PrepareContentInternalAsync(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return ContentOperationResult<GameManifest>.CreateFailure($"Cannot deliver content for manifest {manifest.Id}");
            }

            var deliveryResult = await Deliverer.DeliverContentAsync(manifest, workingDirectory, progress, cancellationToken);
            if (!deliveryResult.Success)
            {
                return ContentOperationResult<GameManifest>.CreateFailure($"Content delivery failed: {deliveryResult.ErrorMessage}");
            }
            ...
            return ContentOperationResult<GameManifest>.CreateSuccess(resultManifest);
        }
        catch (Exception ex)
        {
            Logger.LogError(ex, "Failed to prepare GitHub content for {ManifestId}", manifest.Id);
            return ContentOperationResult<GameManifest>.CreateFailure($"GitHub content preparation failed: {ex.Message}");
        }
    }
}
