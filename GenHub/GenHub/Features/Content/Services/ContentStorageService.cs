...
public class ContentStorageService : IContentStorageService
{
    ...
    public ContentStorageService(ILogger<ContentStorageService> logger, IConfigurationProvider configurationProvider, ICasService casService, IFileHashProvider hashProvider)
    {
        _logger = logger;
    ...
    public string GetContentDirectoryPath(string manifestId) =>
        Path.Combine(_storageRoot, "Data", manifestId); // This path is now legacy or for non-CAS content.

    /// <inheritdoc/>
    public async Task<ContentOperationResult<GameManifest>> StoreContentAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return ContentOperationResult<GameManifest>.CreateSuccess(updatedManifest);
        }
        catch (Exception ex)
        {
            ...
            return ContentOperationResult<GameManifest>.CreateFailure($"Storage failed: {ex.Message}");
        }
    }
    ...
    private async Task<GameManifest> StoreContentFilesInCasAsync(
        GameManifest manifest,
        string sourceDirectory,
