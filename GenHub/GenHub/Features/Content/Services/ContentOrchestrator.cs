...
public class ContentOrchestrator : IContentOrchestrator
{
    ...
    public async Task<ContentOperationResult<GameManifest>> GetContentManifestAsync(
        string providerName, string contentId, CancellationToken cancellationToken = default)
    {
        ...
        var cachedManifest = await _cache.GetAsync<GameManifest>(cacheKey, cancellationToken);
        if (cachedManifest != null)
        {
            _logger.LogDebug("Returning cached manifest for {ProviderName}::{ContentId}", providerName, contentId);
            return ContentOperationResult<GameManifest>.CreateSuccess(cachedManifest);
        }

        var provider = _providers.FirstOrDefault(p => p.SourceName == providerName);
        if (provider == null)
        {
            return ContentOperationResult<GameManifest>.CreateFailure($"Provider not found: {providerName}");
        }
    ...
    public async Task<ContentOperationResult<GameManifest>> ResolveManifestAsync(
        ContentSearchResult contentSearchResult, CancellationToken cancellationToken = default)
    {
        ...
        {
            return ContentOperationResult<GameManifest>.CreateFailure(
        ...
        if (manifestResult.Success && manifestResult.Data != null)
        {
            ...
            if (!validationResult.IsValid)
            {
                ...
                return ContentOperationResult<GameManifest>.CreateFailure($"Manifest validation failed: {error}");
            }

            return ContentOperationResult<GameManifest>.CreateSuccess(manifestResult.Data);
        }
    ...
    /// <summary>
    /// Acquires content and stores GameManifest in pool for later profile usage.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var embeddedManifest = searchResult.GetData<GameManifest>();
            ...
            if (!validationResult.IsValid)
            {
                ...
                if (errors.Any())
                {
                    ...
                    return ContentOperationResult<GameManifest>.CreateFailure($"Manifest validation failed: {errorMessage}");
                }
            }
            ...
            try
            {
                ...
                return ContentOperationResult<GameManifest>.CreateSuccess(prepareResult.Data);
            }
            finally
            {
        ...
        catch (Exception ex)
        {
            ...
            return ContentOperationResult<GameManifest>.CreateFailure($"Content acquisition failed: {ex.Message}");
        }
    }
    ...
    public async Task<ContentOperationResult<IEnumerable<GameManifest>>> GetAcquiredContentAsync(
        CancellationToken cancellationToken = default)
    {
        var manifests = await _manifestPool.GetAllManifestsAsync(cancellationToken);
        return ContentOperationResult<IEnumerable<GameManifest>>.CreateSuccess(manifests);
    }
