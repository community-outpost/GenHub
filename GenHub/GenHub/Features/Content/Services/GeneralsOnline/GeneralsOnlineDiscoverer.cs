    ...
    IHttpClientFactory httpClientFactory) : IContentDiscoverer
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Use provider definition if supplied, otherwise load it
            var providerDef = provider ?? providerLoader.GetProvider(GeneralsOnlineConstants.PublisherType);
            if (providerDef == null)
            {
                logger.LogError("Provider definition not found for {ProviderId}", GeneralsOnlineConstants.PublisherType);
                return OperationResult<IEnumerable<ContentSearchResult>>.CreateFailure(
            ...
            var catalogContent = await FetchCatalogDataAsync(providerDef, cancellationToken);
            ...
            var parser = catalogParserFactory.GetParser(providerDef.CatalogFormat);
            if (parser == null)
            {
                logger.LogError("No parser found for catalog format '{Format}'", providerDef.CatalogFormat);
                return OperationResult<IEnumerable<ContentSearchResult>>.CreateFailure(
                    $"No catalog parser registered for format '{providerDef.CatalogFormat}'");
            }

            // Step 3: Parse the catalog content
            var parseResult = await parser.ParseAsync(catalogContent, providerDef, cancellationToken);
            ...
            {
                // Normalize search term by removing spaces for more flexible matching
                var normalizedSearchTerm = query.SearchTerm.Replace(" ", string.Empty);
                results = results.Where(r =>
                    ...
                    (r.Name?.Replace(" ", string.Empty).Contains(normalizedSearchTerm, StringComparison.OrdinalIgnoreCase) ?? false));
            }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            httpClient.Timeout = TimeSpan.FromSeconds(provider?.Timeouts?.CatalogTimeoutSeconds ?? 30);
