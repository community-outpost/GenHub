...
public class FileSystemDeliverer : IContentDeliverer
{
    ...
    public FileSystemDeliverer(ILogger<FileSystemDeliverer> logger, IConfigurationProvider configProvider)
    {
        _logger = logger;
    ...
    /// <inheritdoc />
    public bool CanDeliver(GameManifest manifest)
    {
    ...
    /// <inheritdoc />
    public async Task<ContentOperationResult<GameManifest>> DeliverContentAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return ContentOperationResult<GameManifest>.CreateSuccess(deliveredManifest);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to deliver local content for manifest {ManifestId}", packageManifest.Id);
            return ContentOperationResult<GameManifest>.CreateFailure($"Content delivery failed: {ex.Message}");
        }
    }

    /// <inheritdoc />
    public Task<ContentOperationResult<bool>> ValidateContentAsync(
        GameManifest manifest, CancellationToken cancellationToken = default)
    {
        try
        {
