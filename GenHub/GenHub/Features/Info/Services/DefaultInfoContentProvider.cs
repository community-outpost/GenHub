...
public class DefaultInfoContentProvider : IInfoContentProvider
{
    ...
    private static InfoSection CreateGeneralsOnlineFAQSection()
    {
        var faqData = new (string Id, string Title, string Content, InfoCardType Type, string Detailed)[]
        {
            ...
            ("faq-15", "Is player privacy protected?", "Yes. Real IP addresses are masked via an overlay relay.", InfoCardType.Feature, "Network traffic routes through a virtual LAN relay by default, masking players' real IP addresses and shielding home networks from direct exposure."),
            ...
            ("faq-20", "Are relays private?", "Yes. Real client IPs are hidden from other players.", InfoCardType.Feature, "Relay servers forward game packets between virtual overlay IPs (10.42.x.x) so peers in the match only see virtual addresses instead of your home IP."),
