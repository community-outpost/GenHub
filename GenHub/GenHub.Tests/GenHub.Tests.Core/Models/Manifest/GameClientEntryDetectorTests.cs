...
public sealed class GameClientEntryDetectorTests : IDisposable
{
    ...
    /// <summary>
    /// Okladnoj layout: the plist names a nested launcher, but the known game binary beside
    /// it wins.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// GeneralsX mac layout: a plist-declared script wins outright over sibling binaries.
    /// </summary>
    [Fact]
    public void DetectEntryPoint_PlistDeclaredScript_WinsOutright()
    {
        var macOs = CreateBundle("GeneralsX.app", declaredExecutable: "run.sh");
        File.WriteAllText(Path.Combine(macOs, "run.sh"), "#!/bin/sh\nexport FOO=1\n");
        WriteBinary(Path.Combine(macOs, "GeneralsXZH"), MachOHeader);

        var resolution = GameClientEntryDetector.DetectEntryPoint(_payload);

        Assert.True(resolution.Success);
        Assert.Equal(Path.Combine("GeneralsX.app", "Contents", "MacOS", "run.sh"), resolution.RelativePath);
    }
    ...
    {
        File.WriteAllBytes(Path.Combine(_payload, "GeneralsXZH.exe"), [0x4D, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        File.WriteAllText(Path.Combine(_payload, "PatchZH.big"), "archive");

        var resolution = GameClientEntryDetector.DetectEntryPoint(_payload);

        Assert.True(resolution.Success);
        Assert.Equal("GeneralsXZH.exe", resolution.RelativePath);
    }
