...
public sealed class UploadHistoryServiceTests : IDisposable
{
    ...
    public async Task CanUploadAsync_WithCategory_AppliesCategoryQuotaAsync()
    {
        ...
        // 9MB profile upload (within 10MB profile limit)
        Assert.True(await service.CanUploadAsync(9 * 1024 * 1024, ProfileSharingConstants.UploadCategoryProfiles));

        // 11MB profile upload (exceeds 10MB profile limit)
        Assert.False(await service.CanUploadAsync(11 * 1024 * 1024, ProfileSharingConstants.UploadCategoryProfiles));
    }
