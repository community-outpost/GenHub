...
public sealed class CrcCatalogUpdateServiceTests : IDisposable
{
    ...
    public async Task CheckForUpdatesAsync_SuccessfulRemoteFetch_UpdatesRegistryAndCacheAsync()
    {
        ...
        // Verify dynamic cache was populated
        dynamicCacheMock.Verify(
            c => c.SetAsync(
                ReplayManagerConstants.CrcCatalogCacheKey,
                It.Is<CrcCatalog>(cat => cat.Mappings.Count == 1),
                It.IsAny<TimeSpan>(),
    ...
    /// <summary>
    /// Verifies that when the catalog is already present in cache, the network request is bypassed.
    /// </summary>
    /// <returns>A task representing the asynchronous unit test.</returns>
    [Fact]
    public async Task CheckForUpdatesAsync_CacheHit_BypassesNetworkAndLoadsRegistryAsync()
    {
        var cachedCatalog = new CrcCatalog
        {
            SchemaVersion = 1,
            TotalEntries = 1,
            Mappings =
        ...
        var dynamicCacheMock = new Mock<IDynamicContentCache>();
        dynamicCacheMock
            .Setup(c => c.GetAsync<CrcCatalog>(ReplayManagerConstants.CrcCatalogCacheKey, It.IsAny<CancellationToken>()))
