...
public class GitHubContentProviderTests
{
    ...
    public GitHubContentProviderTests()
    {
        ...
        _validatorMock.Setup(v => v.ValidateManifestAsync(It.IsAny<GameManifest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ValidationResult("test", new List<ValidationIssue>()));
        _validatorMock.Setup(v => v.ValidateContentIntegrityAsync(It.IsAny<string>(), It.IsAny<GameManifest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ValidationResult("test", new List<ValidationIssue>()));
    ...
    public async Task SearchAsync_OrchestratesDiscoveryAndResolution_Successfully()
    {
        ...
        var resolvedManifest = new GameManifest { Id = "gh.test.mod", Name = "Resolved Test Mod" };
        ...
        _resolverMock.Setup(r => r.ResolveAsync(discoveredItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(ContentOperationResult<GameManifest>.CreateSuccess(resolvedManifest));
        ...
        Assert.NotNull(searchResult.GetData<GameManifest>()); // Manifest should be embedded
    ...
    {
        // Arrange
        var manifest = new GameManifest { Id = "gh.test.mod", Files = new List<ManifestFile>() };
        var deliveredManifest = new GameManifest { Id = "gh.test.mod", Files = [new ManifestFile { RelativePath = "file.txt" }] };
        var targetDirectory = Path.GetTempPath();

        _delivererMock.Setup(d => d.CanDeliver(It.IsAny<GameManifest>())).Returns(true);
        _delivererMock.Setup(d => d.DeliverContentAsync(It.IsAny<GameManifest>(), It.IsAny<string>(), It.IsAny<IProgress<ContentAcquisitionProgress>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(ContentOperationResult<GameManifest>.CreateSuccess(deliveredManifest));
        ...
        _delivererMock.Verify(d => d.CanDeliver(It.IsAny<GameManifest>()), Times.AtLeastOnce());
        _delivererMock.Verify(d => d.DeliverContentAsync(It.IsAny<GameManifest>(), It.IsAny<string>(), It.IsAny<IProgress<ContentAcquisitionProgress>>(), It.IsAny<CancellationToken>()), Times.Once());
    }
}
