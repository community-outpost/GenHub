...
public class BaseContentProviderTests
{
    ...
    public async Task PrepareContentAsync_ValidatesManifestBeforePreparation()
    {
        ...
        var manifest = new GameManifest { Id = "test", Name = "Test" };
    ...
    private class TestContentProvider : BaseContentProvider
    {
        ...
        public override Task<ContentOperationResult<GameManifest>> GetContentAsync(string contentId, CancellationToken cancellationToken = default)
        {
            var manifest = new GameManifest { Id = contentId, Name = $"Content {contentId}" };
        ...
        protected override Task<ContentOperationResult<GameManifest>> PrepareContentInternalAsync(
            GameManifest manifest, string workingDirectory, IProgress<ContentAcquisitionProgress>? progress, CancellationToken cancellationToken)
        {
            return Task.FromResult(ContentOperationResult<GameManifest>.CreateSuccess(manifest));
        }
    }
