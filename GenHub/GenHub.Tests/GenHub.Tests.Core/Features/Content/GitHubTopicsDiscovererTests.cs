...
public class GitHubTopicsDiscovererTests
{
    ...
    public async Task DiscoverAsync_SameTypeAssets_ShareSingleVariantGroupAsync()
    {
        ...
        var first = items[0];
        var second = items[1];
        Assert.Equal("github.community-author.ZeroHourMac.v1.0", first.VariantGroupId);
    ...
    /// <summary>
    /// A multi-platform client release shaped like the acceptance releases splits one
    /// card per asset with per-asset game types, sharing a single variant group.
    /// </summary>
    /// <returns>A task representing the asynchronous test operation.</returns>
    [Fact]
    public async Task DiscoverAsync_MultiPlatformClientRelease_SplitsPerAssetWithGameTypesAsync()
    {
        var discoverer = CreateDiscoverer(
            "GeneralsX",
            "1.0.0",
        ...
        Assert.All(items, item => Assert.Equal(ContentType.GameClient, item.ContentType));
        Assert.All(items, item => Assert.Equal(items[0].VariantGroupId, item.VariantGroupId));

        foreach (var item in items)
        {
            var assetName = item.ResolverMetadata["asset-name"];
            var expected = assetName.Contains("ZH") ? GameType.ZeroHour : GameType.Generals;
            Assert.Equal(expected, item.TargetGame);
        }
    }

    /// <summary>
    /// Release notes beside real content neither force a split nor become cards.
    /// </summary>
    /// <returns>A task representing the asynchronous test operation.</returns>
    [Fact]
    public async Task DiscoverAsync_NotesBesideContent_ExcludedFromCardsAsync()
    {
        var discoverer = CreateDiscoverer(
            "ZeroHourMac",
            "v1.0",
    ...
    private static GitHubTopicsDiscoverer CreateDiscoverer(string repoName, string tag, params string[] assetNames)
    {
        var repo = new GitHubRepositorySearchItem
        {
            Id = 1,
            Name = repoName,
            FullName = $"community-author/{repoName}",
            Owner = new GitHubSearchOwner { Login = "community-author" },
            Topics = [GitHubTopicsConstants.GenHubTopic, GitHubTopicsConstants.GameClientTopic],
            HtmlUrl = $"https://github.com/community-author/{repoName}",
        };
