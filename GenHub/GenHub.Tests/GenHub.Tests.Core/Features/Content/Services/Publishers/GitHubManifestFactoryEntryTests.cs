...
public sealed class GitHubManifestFactoryEntryTests : IDisposable
{
    ...
    {
        File.WriteAllBytes(Path.Combine(_tempDirectory, "GeneralsXZH.exe"), [0x4D, 0x5A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);

        var result = await _factory.CreateManifestsFromExtractedContentAsync(ClientManifest(), _tempDirectory);

        Assert.True(result.Success);
        Assert.Equal("GeneralsXZH.exe", result.Data!.Single().EntryPoint);
    }
