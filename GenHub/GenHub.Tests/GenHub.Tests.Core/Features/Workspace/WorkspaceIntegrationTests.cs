...
public class WorkspaceIntegrationTests
{
    ...
    public async Task EndToEndWorkspaceCreation_AllStrategies(WorkspaceStrategy strategy)
    {
        ...
        if ((strategy == WorkspaceStrategy.SymlinkOnly || strategy == WorkspaceStrategy.HardLink || strategy == WorkspaceStrategy.HybridCopySymlink) && (!isWindows || !isAdmin))
        {
            return;
        }
    ...
    public async Task PrepareWorkspaceAsync_CreatesDirectory()
    {
        ...
        var strategy = new HybridCopySymlinkStrategy(fileOps.Object, logger.Object);
        var managerLogger = new Mock<ILogger<WorkspaceManager>>();
        var manager = new WorkspaceManager(new[] { strategy }, managerLogger.Object);
        ...
        var actual = Path.GetFullPath(Path.GetDirectoryName(info.WorkspacePath) ?? string.Empty).TrimEnd(Path.DirectorySeparatorChar);
    ...
    private static Task VerifyWorkspaceStrategy(WorkspaceInfo workspace, WorkspaceStrategy strategy)
    {
        ...
        switch (strategy)
        {
            case WorkspaceStrategy.FullCopy:
                Assert.False(fileInfo.LinkTarget != null);
                break;
            case WorkspaceStrategy.SymlinkOnly:
                Assert.True(fileInfo.LinkTarget != null);
                break;
        }
