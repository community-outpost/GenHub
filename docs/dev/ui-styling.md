...
5. **Toast notifications for user feedback.** Never create ad-hoc status labels, status bars, or inline `StatusMessage` TextBlocks to report success, failure, or action completion. All user feedback, operation completions, warnings, and errors must be dispatched through `INotificationService` as toast notifications.
