...
## Settings UI: Cloud Storage & Uploads Management
...
- Purge expired local history records (local history entries older than 30 days are automatically purged from tracking, though uploaded files remain hosted until explicitly deleted or removed by retention policies).
