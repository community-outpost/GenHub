...
```json
{
  ...
  "contentRegistryFile": "communityoutpost.content-registry.json",
  "enabled": true,
  "endpoints": {
...
| `contentRegistryFile` | string | Path to content registry JSON, loaded by `IProviderDefinitionLoader.LoadContentRegistryAsync()` |
...
3. **Registry Loading** - `IProviderDefinitionLoader.LoadContentRegistryAsync()` loads metadata from `contentRegistryFile`
4. **Parsing** - Parser transforms catalog content using registry for metadata lookup

```csharp
// In CommunityOutpostDiscoverer.DiscoverAsync():
var parser = _catalogParserFactory.GetParser(provider.CatalogFormat);
var registry = await _providerLoader.LoadContentRegistryAsync(
    provider.ProviderId, provider.ContentRegistryFile, cancellationToken);

var results = await parser.ParseAsync(catalogContent, provider, registry, cancellationToken);
```
...
public interface ICatalogParser
{
    ...
    /// Metadata is sourced from domain-specific registries (e.g., GenPatcherContentRegistry).
    /// </summary>
...
public class MyNewCatalogParser : ICatalogParser
{
    ...
    public async Task<OperationResult<IEnumerable<ContentSearchResult>>> ParseAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Parse catalogContent using provider.Endpoints.Mirrors
        // Look up metadata from a domain-specific registry class
