...
```
CustomIcons/
├── CustomIcons.mbproj                               # ModBuilder project descriptor
...
│   └── ModBundlePacks.json                         # Packages files into .Release/!CustomIconsAndHotkeys.big
│
└── GameFilesEdited/
    ├── Art/
    │   └── Textures/
...
2. Click **Open Project** and select `CustomIcons.mbproj` (or select it from the recent projects list).
...
4. Select build options:
   ...
   - Check **Release** (generates `.Release/!CustomIconsAndHotkeys.big`).
5. Click **Execute Build**.
6. The compiled archive will be placed in `.Release/!CustomIconsAndHotkeys.big`.
...
1. Copy `.Release/!CustomIconsAndHotkeys.big` to your Zero Hour game root folder (e.g. `C:\Games\Command and Conquer Generals Zero Hour\`).
2. Launch Zero Hour.
