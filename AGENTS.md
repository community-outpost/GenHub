...
- **Scrollable Views & Bottom Viewport Clearance (Mandatory Anti-Clipping Rule):**
  - **NEVER set `Padding` on `<ScrollViewer>`:** Avalonia UI (11.x) has a known layout bug where `ScrollContentPresenter` does NOT add bottom/right padding to the scrollable `Extent`. This cuts off bottom content and buttons, preventing users from scrolling to them.
  - **ALWAYS apply margins to the inner content container:** Keep `<ScrollViewer>` clean without padding, and set `Margin="24,24,24,64"` on the direct child container (e.g., `<StackPanel Spacing="16" Margin="24,24,24,64">`).
  - **ALWAYS reserve at least 48px to 64px bottom clearance:** `MainView.axaml` anchors a floating version button at the bottom-right corner (`HorizontalAlignment="Right" VerticalAlignment="Bottom" Margin="0,0,10,5"`). Views without a 48-64px bottom margin will have their lowest buttons, cards, or notices cut off by the window border or covered by the version watermark.
...
- **Scrollable View Layout (Anti-Clipping & Overlay Clearance):**
  - Never write `<ScrollViewer Padding="...">`. Always leave `ScrollViewer` unpadded and put `Margin="24,24,24,64"` (minimum 48-64px bottom margin) on the direct inner container (`StackPanel`, `Border`, etc.).
  - Why: (1) Avalonia's `ScrollContentPresenter` omits bottom padding from the scroll extent, permanently clipping bottom buttons. (2) `MainView` has a persistent floating version badge in the bottom-right corner; the 48-64px bottom buffer ensures interactive buttons and text are never covered.
